...
첫 실행 시 **설정** 화면에서 환경별 `baseUrl` 과 관리키(`secret$token`)를 등록해야 한다.
...
`$` 앞이 서명 secret 이고, 뒤가 **관리 토큰**(HS256 JWT)이다.
...
게이트웨이로 나가는 `X-API-KEY` 는 **토큰 부분만**이다 (`config::resolve`). secret 은 서명
검증에만 쓰이고 앱 밖으로 나가지 않는다.

`$` 가 없는 문자열은 관리키가 아니다 — 저장 단계에서 거부하고, 이미 저장돼 있었다면
잠금 화면이 형식을 알려 준다.
...
| | 전체 관리자 (`admin`) | service 지정 |
|---|---|---|
| 아이콘 레일 | 6개 전부 | **Upstream · Service 없음** |
| Route 목록 | 전부 | 권한 있는 service 에 연결된 route 만 |
| Route 상세 · Import · name 접두사의 service 콤보 | 전부 | 권한 있는 service 만 |
| chip 건수 · 좌측 패널 집계 · 대시보드 route KPI | 전부 | 목록과 같은 범위 |
| 설정 화면 | + 관리키 발급 카드 | 접속 정보와 내 권한 확인만 |
...
`perm::resolve(env)` — 저장된 관리키를 쪼개(`split`) 서명 · `key` · 기간을 확인하고 `Perm` 을
준다. `Perm::None` 이 되는 사유는 일곱 가지고(관리키 없음 · `secret$token` 아님 · JWT 아님 ·
서명 불일치 · key 불일치 · 시작 전 · 만료), 잠금 화면 문구가 그 사유로 갈린다.
...
### 업그레이드 주의 — `$` 없는 기존 토큰은 잠긴다

예전 버전은 서명 secret 을 코드 상수로 들고 있었으므로 저장값이 JWT 한 줄이었다. 이제
secret 이 코드에 없으니 그 토큰은 검증할 방법 자체가 없다 — `secret$token` 이 아닌 저장값은
`Perm::None` 이라 **잠금 화면**이 뜬다. 배포 시 사용자 전원에게 관리키를 재발급해
전달해야 한다.

잠금 화면이 사유를 정확히 말해 주므로("관리키가 secret$token 형식이 아닙니다") 문의가
원인 파악에서 막히지는 않는다. 새 관리키를 입력란에 붙여넣으면 그대로 풀린다.
...
- 관리키는 **Windows 자격 증명 관리자**에 저장한다 (`api-gateway-manager` / `apisix-admin-token::dev|prod`).
  디스크에 평문으로 남기지 않는다.
- **서명 secret 은 저장소에도 바이너리에도 없다.** 관리키(`secret$token`)가 실어 오고,
  게이트웨이로는 그 중 토큰만 `X-API-KEY` 로 나간다 (`config::resolve`).
- 마스킹 문자열(`config::mask`)은 관리키가 아니라 **토큰** 앞자리를 자른다. 관리키의 앞부분이
  곧 secret 이라, 예전 규칙대로 앞 6자를 보여 주면 설정 화면과 대시보드가 secret 조각을
  뿌리게 된다. 회귀 테스트로 못 박아 두었다 (`mask_never_leaks_the_secret`).
...
- 실제 인가는 **게이트웨이**가 한다. 토큰은 `X-API-KEY` 로 그대로 나가고, 무엇을 허용할지는
  APISIX 가 정한다. 앱이 커맨드마다 권한을 한 번 더 확인하는 것(`commands::require_admin`)은
  방어라기보다 "화면과 IPC 가 서로 다른 말을 하지 않게" 하기 위한 것이다.
