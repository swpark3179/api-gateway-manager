...
첫 실행 시 **설정** 화면에서 환경별 `baseUrl` 과 관리키를 등록해야 한다. 입력한 관리키는
...
├─ jwt.rs                   HS256 서명 (Consumer 토큰 · 관리 토큰)
├─ perm.rs                  ★ 관리키 분해 · 관리 토큰(JWT) 해석 · 권한 판정 (단일 진입점)
...
스펙 조회도 게이트웨이와 **똑같이 항상 시스템 프록시를 우회한다.** 예전에는 Import 화면에
`프록시 우회` 체크박스를 두고 이것만 뒤집을 수 있게 했는데, 사내에서는 우회가 켜져 있어야
스펙 주소에도 닿으므로 끌 이유가 없었다. `EnvConfig.no_proxy` 를 `config::force_toggles` 가
항상 `true` 로 강제하는 것과 같은 판단이다 — 고를 수 없는 값을 스위치로 보여 주지 않는다.

`http` / `https` 스킴만 허용하고(주소가 임의 파일 읽기가 되지 않도록) 응답은 8 MiB 로 제한한다.
Service 폼도 저장 시점에 같은 스킴 제한을 걸어, 나중에 `fetch_text` 가 거부할 값이 애초에
저장되지 않게 한다.

실패 문구는 게이트웨이 경로와 **갈라 둔다** (`AppError::from_spec_status`). 스펙 조회는
`X-API-KEY` 를 붙이지 않으므로 그 401 / 403 은 관리키와 아무 관계가 없는데, 게이트웨이용
분류를 그대로 쓰면 "관리 토큰이 올바르지 않습니다" 가 떠서 Import 화면에서 service 를 무엇으로
바꿔도 같은 문구가 나오고 멀쩡한 관리키를 의심하게 된다. 401 / 403 만 다르고 나머지 상태
코드의 분류는 `from_status` 를 그대로 재사용한다.
...
사용자가 붙여넣고 자격 증명 관리자에 저장되는 문자열을 **관리키**라 부른다. 게이트웨이로는
**입력한 그대로** 나간다 — 자르거나 조립하지 않는다 (`config::api_key` → `config::resolve`).

무엇이 유효한 `X-API-KEY` 인지 아는 것은 게이트웨이뿐이다. 앱이 그 값을 해석해 일부만
보내면 관리키의 모양을 앱이 규정하는 셈이 되고, 그 모양에 맞지 않는 관리키로는 **연결조차**
할 수 없게 된다. 관리키 형식 검사는 저장 단계에서도 하지 않는다.

### `secret$token` 이면 권한도 읽는다

관리키가 `secret$token` 모양이고 뒤가 우리 관리 토큰(HS256 JWT)이면, `$` 앞의 secret 으로
서명을 확인하고 payload 의 기간·권한을 읽어 메뉴·목록·콤보박스를 줄인다.

```
edd1c9f034335f136f87ad84b625c8f1$eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJrZXkiOi…
└──────────── secret ───────────┘└──────────────── 관리 토큰 ────────────────┘
└──────────────────────── 이 줄 전체가 X-API-KEY ─────────────────────────────┘
```

secret 을 소스나 바이너리에 상수로 두지 않기 위한 형식이다 — 저장소와 실행 파일만으로는
아무 토큰도 서명할 수 없다. 쪼개고 합치는 규칙은 `perm::split` · `perm::join` 한 쌍이
도맡는다. 구분자 `$` 는 base64url 알파벳에 없어 토큰 쪽에는 나타나지 않으므로 **마지막**
`$` 로 나눈다.

#### 해석하지 못한 관리키 — `Perm::Opaque`

그 모양이 아니거나(평범한 admin key 한 줄), `$` 뒤가 JWT 가 아니거나, 서명·`key` 클레임이
맞지 않으면 기간도 권한도 읽을 수 없다. 그때가 `Perm::Opaque` 다.

- **잠기지 않는다.** 관리키는 그대로 나가고 연결도 그대로 된다.
- **앱은 아무것도 제한하지 않는다** — `is_admin()` · `allows()` · `allow_blob()` 이 전체
  관리자와 같다. 읽지 못한 것을 "권한 없음" 으로 취급하면 그런 관리키로는 아무것도 할 수
  없는데, 정작 게이트웨이에서는 전체 관리자일 수 있다. 무엇을 허용할지는 어차피
  게이트웨이가 정한다 (아래 '관리 권한은 보안 경계가 아니다').
- **설정 화면만 다르다.** `kind: "opaque"` 로 내려가 기간·권한 칸 대신 "무엇을 읽지
  못했는지" 한 줄을 보여 준다. 오류 문구가 아니다.

### 토큰의 클레임

payload 에 유효기간과 권한이 실려 있고, 앱은 그것을 해석해 메뉴 · 목록 · 콤보박스를 줄인다.

```json
{ "key": "…", "nbf": 1767225600, "exp": 1798761599, "perm": "admin" }
{ "key": "…", "nbf": 1767225600, "exp": 1798761599, "perm": ["svc-order", "svc-pay"] }
```

| 클레임 | 뜻 |
|---|---|
| `key` | 고정값. `perm::ADMIN_KEY` 와 다르면 우리 관리 토큰이 아니다 |
| `nbf` | 시작일 (unix 초). 이전이면 잠금 |
| `exp` | 종료일 (unix 초). 이후면 잠금 |
| `perm` | `"admin"` = 전체 관리자, 또는 관리 권한이 있는 **service id 목록** |

`nbf` · `exp` 를 표준 클레임으로 쓴 이유: APISIX 의 `jwt-auth` 플러그인이 이 둘을 자체
검증하므로, 앱이 통과시킨 만료 토큰이라도 게이트웨이에서 한 번 더 걸린다.

### 권한별로 달라지는 것

| | 전체 관리자 (`admin`) | service 지정 | 해석 불가 (`opaque`) |
|---|---|---|---|
| 아이콘 레일 | 6개 전부 | **Upstream · Service 없음** | 6개 전부 |
| Route 목록 | 전부 | 권한 있는 service 에 연결된 route 만 | 전부 |
| Route 상세 · Import · name 접두사의 service 콤보 | 전부 | 권한 있는 service 만 | 전부 |
| chip 건수 · 좌측 패널 집계 · 대시보드 route KPI | 전부 | 목록과 같은 범위 | 전부 |
| 설정 화면 | + 관리키 발급 카드 | 접속 정보와 내 권한 확인만 | + 발급 카드, 기간·권한 칸은 사유 한 줄 |

Upstream · Service 를 숨기는 이유는 둘이 게이트웨이 전체에 걸친 리소스라는 데 있다 —
특정 service 만 맡은 토큰이 upstream 을 고치면 자기 권한 밖의 route 까지 영향을 받는다.
다만 **조회는 막지 않는다**: Service 콤보의 `(host:port)` 라벨이 upstream 캐시에서 오므로,
부트스트랩이 그것을 돌려야 Route 폼의 service 목록이 온전해진다. 변경만 admin 전용이다.

#### 게이트웨이가 목록 조회를 거부할 수 있다

앱이 화면을 줄이는 것과 별개로, **게이트웨이가 이 토큰의 `GET /services` · `GET /upstreams` 를
거부할 수 있다** (진짜 인가 경계는 그쪽이다). 여기에 두 가지 대비가 들어가 있다.

- **콤보 목록은 토큰의 `perm` 클레임과 교차한다** (`store.serviceOptions`). `services` 배열은
  이미 Rust 가 걸러 내려보내지만(`commands::services_cached`), 부트스트랩은 성공했을 때만
  배열을 갈아 끼우므로 조회가 거부되면 **직전 토큰이 받아 둔 목록이 그대로 남는다.** 그래서
  프런트에서 한 번 더 교차하고, 클레임에는 있는데 배열에 없는 service 는 최소 항목으로 만들어
  붙인다 — 목록 조회가 막혀도 자기 service 는 고를 수 있어야 한다. 합성 항목은 `spec_url` 을
  모르므로 Import 화면이 "등록되지 않았다" 와 다르게 안내한다.
  관리키를 바꾸거나 baseUrl 을 고치면 `saveSettings` 가 직전 토큰으로 받아 둔 것을 전부 버린다.
- **부가 단계 실패는 새로고침을 실패로 만들지 않는다.** 부트스트랩의 다섯 단계는
  필수(Route · Consumer)와 부가(Upstream · Service · 접근 집계)로 갈린다 (`BootStep.essential`).
  부가 실패는 `error` 가 아니라 `bootAux` 로 모여 성공 토스트 뒤에 "무엇을 조회하지 못했는지" 로
  붙는다. 예전에는 이 401 이 첫 에러로 올라와, Route · Consumer 가 멀쩡히 갱신됐는데도 상단
  새로고침이 "관리 토큰이 올바르지 않습니다" 만 띄웠다. 필수 단계가 실패하면 **어느 조회였는지**
  를 문구 앞에 붙인다 — 게이트웨이가 주는 401 은 전부 같은 문장이라 그것만으로는 구별되지 않는다.
  (Rust `meta::overview` 가 대시보드에서 이미 같은 판단을 하고 있다)

### 판정하는 곳은 한 군데다

`perm::resolve(env)` — 저장된 관리키를 `perm::from_key` 에 넘겨 `Perm` 을 받는다. 그 안에서
쪼개고(`split`) 서명 · `key` · 기간을 확인한다.

**잠기는 것은 두 가지뿐이다**: 관리키가 아예 없거나(`NoToken` — 보낼 값이 없다), 읽어 낸
토큰이 스스로 기간 밖이라고 말하거나(`NotYet` · `Expired` — `jwt-auth` 도 같은 판정을 한다).
나머지 네 사유(`secret$token` 아님 · JWT 아님 · 서명 불일치 · key 불일치)는 `Perm::Opaque`
이고 연결을 막지 않는다. 잠금 화면과 설정 화면이 어긋나지 않도록 그 판단은 `from_key`
한 곳에서만 갈린다.

기간 검사는 `decode` 가 아니라 `at` 이 따로 한다. 만료된 토큰도 설정 화면에서 시작일 ·
종료일 · 권한 service 를 보여 줘야 하기 때문이다 — 그러지 않으면 사용자는 자기가 왜
막혔는지 확인할 방법이 없다.

### 목록을 좁히는 방식 — 조회의 다섯째 축

Route 조회는 이미 네 축(상태 chip · 검색어 · name 접두사 · 조회 범위)을 SQL 에서 합친다.
관리 권한은 그 옆에 붙는 **다섯째 축**이고, 프런트에서 오지 않는다 —
`commands.rs` 가 `perm::resolve` 로 만들어 붙인다.

```sql
AND (?6 = '' OR instr(?6, char(10) || service_id || char(10)) > 0)
```

`?6` 은 `Perm::allow_blob()` 이 만든다. `""` 는 제한 없음(admin)이고, 그 외에는
`"\nsvc-a\nsvc-b\n"` 이라 `instr` 이 `"\n" || service_id || "\n"` 을 찾으면 접두사가 같은
id 끼리 서로 걸리지 않는다.

`service_id IN (?,?,…)` 을 쓰지 않는 이유는 `ACCESS_CLAUSE` 와 같다: 플레이스홀더 개수가
조회마다 달라지면 `route_counts` 와 바인딩 수가 어긋나 rusqlite 가 런타임에
`InvalidParameterCount` 를 던진다.

허용목록에 걸린 함정 두 개는 회귀 테스트로 못 박아 두었다
(`service_allowlist_narrows_every_route_count`):

- **빈 목록은 `"\n"` 이어야 한다.** `format!("\n{}\n", "")` 는 `"\n\n"` 을 주는데, 그것은
  service 가 없는 라우트(`service_id = ''`)를 찾는 바늘과 정확히 같다 — 권한이 하나도 없는
  토큰에게 그 라우트들이 보이게 된다.
- **좌측 패널 집계의 허용목록은 `ON` 절에 둔다.** `WHERE` 로 옮기면 권한 밖 라우트만 가진
  컨슈머의 모든 행이 걸러져 `GROUP BY` 에서 그 컨슈머 자체가 사라진다.
  패널은 "접근 0건" 을 보여 줘야 하고, 행이 없는 것과 0 은 다르다.

### 개발 / 운영 전환은 홈으로

개발이 전체 관리자이고 운영이 아닐 수 있다. Service 화면을 보던 중 운영으로 넘어가면 그
화면은 접근 권한 밖이 된다. `setEnv` 가 `section: "dash"` 로 되돌려 그 상태 자체를 없앤다.
토큰을 저장해 권한이 줄어든 경우도 같다 (`saveSettings`).

레일에서 숨기는 것만으로는 부족하므로 `go()` 도 `ADMIN_ONLY` 를 한 번 더 보고,
커맨드도 각자 확인한다 (`require_admin` · `require_service`).

### 발급

전체 관리자면 설정 화면에 `관리키 발급` 카드가 뜬다. **secret** · 시작일 · 종료일 ·
권한(전체 관리자 / service 지정)을 넣어 관리키를 만든다. 결과는 `secret$token` 한 줄이고,
그것을 그대로 사용자에게 전달한다 — 토큰만 건네면 secret 이 빠져 쓸 수 없다.

secret 은 발급자가 **직접 입력**한다. 화면에서 새로 만들어 주지 않는 이유는, 게이트웨이의
관리 consumer 에 등록된 `jwt-auth.secret` 과 달라지면 발급한 관리키가 게이트웨이에서
거부되기 때문이다. 값을 확인하거나 새로 만드는 자리는 Consumer 화면이다.
환경을 전환하면 입력한 secret 을 비운다 — 개발 secret 으로 운영 관리키를 발급하면
그 사실이 배포한 뒤에야 드러난다.

**서명만 한다** — 실제로 쓰려면 게이트웨이에 같은 key · secret 의 consumer 가 등록돼
있어야 한다.

service id 의 정렬 · 중복 제거는 `jwt::sign_admin` 안에서 한다. 호출부에 맡기면 같은 권한을
고른 두 사람이 서로 다른 토큰을 받는다 (payload 키 순서를 손으로 맞추는 것과 같은 이유).
빈 secret 과 `$` 가 섞인 secret 도 여기서 거부한다 — `$` 는 관리키 구분자다.

### 업그레이드 주의 — `$` 없는 기존 토큰

예전 버전은 서명 secret 을 코드 상수로 들고 있었으므로 저장값이 JWT 한 줄이었다. 이제
secret 이 코드에 없으니 그 토큰은 검증할 방법 자체가 없다 — `secret$token` 이 아닌 저장값은
`Perm::Opaque` 다.

**잠기지는 않는다.** 그 값이 그대로 `X-API-KEY` 로 나가므로, 게이트웨이가 받아 주는
값이었다면 계속 쓸 수 있다. 다만 기간·권한을 읽지 못하므로 앱은 화면을 줄이지 않고,
설정 화면이 그 사실을 한 줄로 알려 준다. 기간제 권한을 실제로 쓰려면 사용자 전원에게
관리키를 재발급해 전달해야 한다 — 새 관리키를 입력란에 붙여넣으면 그때부터 권한이 읽힌다.

(예전 버전은 이 상태를 `Perm::None` 으로 보고 잠금 화면을 띄웠다. 그 판정이 `X-API-KEY` 를
`$` 뒤로 자르던 규칙과 짝이었는데, 자르지 않게 되면서 잠글 이유도 없어졌다.)

> **예전 secret 은 게이트웨이에서 교체해야 한다.** 상수를 지운 것으로 이미 커밋된 이력이
> 사라지지는 않는다 — 예전 값은 git 히스토리에 그대로 남아 있으므로, 그것을 아는 사람은
> 여전히 게이트웨이가 받아 주는 토큰을 만들 수 있다. Consumer 화면에서 관리 consumer 의
> `jwt-auth.secret` 을 **새 값으로 바꾸고**, 그 값으로 관리키를 재발급해야 이 변경의
> 효과가 실제로 생긴다.
...
- 관리키는 **Windows 자격 증명 관리자**에 저장한다 (`api-gateway-manager` / `apisix-admin-token::dev|prod`).
  디스크에 평문으로 남기지 않는다.
- **서명 secret 은 저장소에도 바이너리에도 없다.** 관리키(`secret$token`)가 실어 온다.
- **게이트웨이로는 관리키 전체가 나간다** (`config::api_key`) — secret 을 포함한 한 줄이
  그대로 `X-API-KEY` 다. 앱이 그 값을 해석해 일부만 보내면 관리키의 모양을 앱이 규정하는
  셈이 되기 때문이다. 사내 게이트웨이 호출은 프록시를 우회해 직접 나가지만
  (`no_proxy` 고정), 게이트웨이 접근 로그에는 그 값이 남는다.
- 화면에 뿌리는 값은 그것과 별개다. 마스킹 문자열(`config::mask`)은 관리키가 아니라
  **토큰** 앞자리를 자른다. 관리키의 앞부분이 곧 secret 이라, 예전 규칙대로 앞 6자를
  보여 주면 설정 화면과 대시보드가 secret 조각을 뿌리게 된다. `secret$token` 형태가
  아니면 아무것도 드러내지 않는다. 회귀 테스트로 못 박아 두었다
  (`mask_never_leaks_the_secret`).
- **설정 화면에 들어오면** `settings_reveal_token` 커맨드가 **원문**(secret 포함)을 준다.
  입력란이 `type="password"` 라 값이 있어야 점으로 보이기 때문이다 — 값을 채우지 않으면
  미표시 모드가 빈 칸이 되어 관리키가 등록돼 있는지조차 알 수 없다. `표시` 는 그 `type` 만
  뒤집는다. 예전에는 `표시` 를 누른 순간에만 원문을 받았으므로, **원문이 웹뷰 메모리에 오는
  시점이 클릭 → 화면 진입으로 앞당겨진 것**이 이 변경의 실질이다. 노출 범위 자체는 같다:
  그 사용자가 이미 가진 자기 값이고, 저장 경로는 그대로 자격 증명 관리자다.
- 손대지 않은 입력값은 저장에서 `null`(유지)로 나간다 — 화면을 열었다 닫는 것만으로 자격
  증명이 다시 쓰이지 않는다. 지우려면 `관리키 삭제` 를 누른다.
- `프록시 강제 우회` · `인증서 검증 건너뛰기` 는 **항상 켜져 있다** — 사내 환경에서 둘 다
  켜져야 게이트웨이에 닿으므로 선택 항목에서 뺐다 (`config.rs` `force_toggles`).
  인증서를 검증하지 않으므로 중간자 공격에 노출될 수 있다. 신뢰된 사내망 전용이며,
  가능하면 사내 CA 를 Windows 인증서 저장소에 설치하는 편이 안전하다
  (reqwest 를 `rustls-tls-native-roots` 로 빌드해 OS 인증서 저장소를 사용한다).

### 관리 권한은 보안 경계가 아니다

아래 '관리토큰 권한' 절의 권한 모델이 하는 일은 **화면을 줄이는 것**뿐이다.

- secret 이 바이너리에서 빠졌어도 이 절의 결론은 그대로다. 관리키가 secret 과 토큰을
  **함께** 실어 오므로, 자기 secret 으로 서명한 관리키는 언제나 자기 검증을 통과한다 —
  앱의 서명 검증은 "붙여넣은 관리키가 스스로 앞뒤가 맞는가" 를 보는 것이고 위조를 막지
  못한다. 막는 것은 게이트웨이다: `jwt-auth` 에 등록된 secret 과 다르면 거부된다.
  바이너리에서 secret 을 뺀 것으로 얻은 것은 "저장소·실행 파일 유출만으로 통하는 토큰을
  만들 수 없다" 이고, 그 이상을 기대해서는 안 된다.
- 실제 인가는 **게이트웨이**가 한다. 관리키는 `X-API-KEY` 로 그대로 나가고, 무엇을
  허용할지는 APISIX 가 정한다. 앱이 커맨드마다 권한을 한 번 더 확인하는
  것(`commands::require_admin`)은 방어라기보다 "화면과 IPC 가 서로 다른 말을 하지 않게"
  하기 위한 것이다.
- 같은 이유로 해석하지 못한 관리키(`Perm::Opaque`)에 제한을 걸지 않는다. 앱이 거기서
  메뉴를 빼도 막히는 것은 화면뿐이고, 정작 그 관리키가 게이트웨이에서 무엇을 할 수 있는지는
  바뀌지 않는다 — 얻는 것 없이 쓸 수 없는 앱만 남는다.
...
| 설정 토글 | 프록시 우회 1개 | **없음 — 프록시 우회·인증서 검증 건너뛰기 모두 항상 ON** | 둘 다 꺼서 되는 사내 환경이 없다. 고를 수 없는 값을 스위치로 두면 "껐는데 왜 그대로냐" 는 질문만 남는다 |
| 설정 화면 | 접속 정보만 | + 관리키가 담은 권한 상세 · 관리키 발급(전체 관리자) | 아래 '관리토큰 권한' 절 |
| 잠금 화면 문구 | "토큰을 등록하세요" 한 문장 | 막힌 사유별 문구 6종 | 이미 등록한 사용자에게 "등록하세요" 는 답이 아니다 (만료·서명 불일치 등) |
