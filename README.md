...
| Route 상세의 service 콤보 | 전부 | 권한 있는 service 만 |
...
- 평소 프런트로는 마스킹된 문자열만 내려가지만, 설정 화면에서 사용자가 `표시` 를 누르면
  `settings_reveal_token` 커맨드가 **원문**을 준다(secret 포함). 자기 관리키를 확인하고
  복사할 수 없으면 표시/가리기 토글이 성립하지 않아 의도적으로 낸 구멍이고, 그 사용자가
  이미 가진 자기 값이다. 저장 경로는 그대로 자격 증명 관리자다.
- 설정 화면의 입력란을 비워 두면 "기존 관리키 유지"로 동작한다. 지우려면 `관리키 삭제` 를 누른다.
