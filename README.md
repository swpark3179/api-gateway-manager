...
├─ perm.rs                  ★ 관리 토큰(JWT) 해석 · 권한 판정 (단일 진입점)
...
{ "key": "apisix-manage", "nbf": 1767225600, "exp": 1798761599, "perm": ["svc-order", "svc-pay"] }
```

| 클레임 | 뜻 |
|---|---|
| `key` | `apisix-manage` 고정. 다르면 우리 관리 토큰이 아니다 |
...
| 설정 화면 | + 관리 토큰 발급 카드 | 접속 정보와 내 권한 확인만 |
...
`perm::resolve(env)` — 저장된 토큰을 읽어 서명 · `key` · 기간을 확인하고 `Perm` 을 준다.
`Perm::None` 이 되는 사유는 여섯 가지고(토큰 없음 · JWT 아님 · 서명 불일치 · key 불일치 ·
시작 전 · 만료), 잠금 화면 문구가 그 사유로 갈린다.
...
전체 관리자면 설정 화면에 `관리 토큰 발급` 카드가 뜬다. 시작일 · 종료일 · 권한
(전체 관리자 / service 지정)을 골라 JWT 를 만든다. **서명만 한다** — 실제로 쓰려면
게이트웨이에 `key=apisix-manage` consumer 가 등록돼 있어야 한다.

service id 의 정렬 · 중복 제거는 `jwt::sign_admin` 안에서 한다. 호출부에 맡기면 같은 권한을
고른 두 사람이 서로 다른 토큰을 받는다 (payload 키 순서를 손으로 맞추는 것과 같은 이유).

### 업그레이드 주의 — 기존 평문 토큰은 잠긴다

예전 버전은 아무 문자열이나 `X-API-KEY` 로 받아 썼다. 이제 JWT 가 아니거나 서명이 맞지
않는 토큰은 `Perm::None` 이라 **잠금 화면**이 뜬다. 배포 시 사용자에게 새 형식의 관리
토큰을 재발급해 전달해야 한다. 잠금 화면이 사유를 정확히 말해 주므로
("관리 토큰이 JWT 형식이 아닙니다") 문의가 원인 파악에서 막히지는 않는다.
...
- 평소 프런트로는 마스킹된 문자열만 내려가지만, 설정 화면에서 사용자가 `표시` 를 누르면
  `settings_reveal_token` 커맨드가 **원문**을 준다. 자기 토큰을 확인하고 복사할 수 없으면
  표시/가리기 토글이 성립하지 않아 의도적으로 낸 구멍이다. 저장 경로는 그대로 자격 증명 관리자다.
...
- 서명 secret(`shi-api-secret`)이 바이너리에 상수로 박혀 있다. 추출하면 누구나 전체 관리자
  토큰을 만들 수 있다. 문자열을 숨기는 것으로 막을 수 있는 문제가 아니라, 애초에 이것을
  인가 경계로 삼지 않는 것이 해법이다.
...
| 설정 화면 | 접속 정보만 | + 토큰이 담은 권한 상세 · 관리 토큰 발급(전체 관리자) | 아래 '관리토큰 권한' 절 |
