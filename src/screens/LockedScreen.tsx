 ...
 *
 * 막히는 이유는 세 가지다 (관리키 미등록 · 사용 시작 전 · 만료). "토큰을 등록하세요"
 * 한 문장으로 뭉치면 이미 등록한 사용자가 무엇을 해야 하는지 알 수 없다.
 * 문구는 Rust `perm::Reason::message` 가 만든다.
 *
 * **해석하지 못한 관리키는 여기 오지 않는다.** `secret$token` 이 아니어서 기간·권한을
 * 읽지 못한 것뿐이면 연결은 되므로 앱을 그대로 쓴다 (Rust `perm::Perm::Opaque`).
 * 설정 화면이 무엇을 못 읽었는지만 알려 준다.
 */
